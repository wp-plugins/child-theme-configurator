...
For WordPress Multisite (Network) users, go to Network Admin > Themes > Child Themes. 
...
v.1.6.3 **IMPORTANT:** Now only users with "install_themes" capability have access to the Child Theme Configurator. **MULTISITE USERS:** The Admin HAS MOVED to Network Admin > Themes > Child Themes for better handling for multisite (network) installs. See changelog for details.
