...
class ChildThemeConfiguratorAdmin {
    ...
    function ajax_save_postdata( $action = 'ctc_update' ) {
        ...
        if ( $this->validate_post( $action ) ):
            ...
            // version 1.7 replacing updates array with direct html response
            include_once( CHLD_THM_CFG_DIR . '/includes/class-ctc-ajax.php' );
            new ChildThemeConfiguratorAjax();
        else:
            die( 0 );
        endif;
    }
    ...
    function ajax_query_css( $action = 'ctc_update' ) {
        ...
        if ( $this->validate_post( $action ) ):
            ...
            if ( !empty( $param[ 'obj' ] ) ):
                ...
                // version 1.7 replacing updates array with direct html response
                include_once( CHLD_THM_CFG_DIR . '/includes/class-ctc-ajax.php' );
                new ChildThemeConfiguratorAjax( $param );
            endif;
        endif;
        die( 0 );
    }
